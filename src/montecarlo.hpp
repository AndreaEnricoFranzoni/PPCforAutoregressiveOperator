...
public:
  ...
  double                              apply(FunPoint const &, const std::vector<double> &inter_val, double const &a, double const &b) const override;
  ...
  double
  cumulatedError() const
  {
    return M_cumulatedError;
  };
  //! Warns if iterations exceeded
  bool
  iterationsExceeded() const
  {
    return M_iterationsExceeded;
  }
