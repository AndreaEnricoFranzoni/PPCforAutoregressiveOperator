...
public:
  ...
  double apply(FunPoint const &f,const std::vector<double> &inter_val,double const &a,
...
apsc::NumericalIntegration::StandardQuadratureRule<N>::apply(
  FunPoint const &f,const std::vector<double> &inter_val,double const &a, double const &b) const
{
  double h2=(b - a) * 0.5; // half length
  double xm=(a + b) * 0.5; // midpoint
  // scale functions
  auto   fscaled = [&h2,&xm,&f,&inter_val](double x) { return f(x * h2 + xm,inter_val); };
  double tmp=0.0;
  auto   np = n_.begin();
