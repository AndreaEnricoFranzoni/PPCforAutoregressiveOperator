...
double
MonteCarlo::apply(FunPoint const &f,const std::vector<double> &inter_val, double const &a, double const &b) const
{
  using namespace std;
  ...
  if(M_iterationsExceeded)
    std::clog << "Montecarlo N. iterations exceeded"
              << " Error " << errore << " Target error " << error_level
              << std::endl;
  return (b - a) * mean;
}
